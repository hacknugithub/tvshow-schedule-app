...
class ShowTvDetailActivity : AppCompatActivity() {
    ...
    private fun loadShow(showId: String?){
        ...
        requestCall.enqueue(object : Callback<MySchedule.Show> {
            override fun onResponse(call: Call<MySchedule.Show>, response: Response<MySchedule.Show>) {
                ...
                if (response.isSuccessful){
                    ...
                    tvDetailRating?.text = String.format(res.getString(R.string.tv_rating_detail),
                        showItem.rating.average
                    )
                    val summarySanitized = Html.fromHtml(showItem.summary).toString()
                    tvShowDetail?.text = String.format(res.getString(R.string.title_activity_tv_show_detail), summarySanitized)
                    tvShowGenre?.text = String.format(res.getString(R.string.tv_genre_detail),
                        showItem.genres.joinToString()
                    )
                    tvShowSchedule?.text = String.format(res.getString(R.string.tv_schedule_detail), showItem.schedule.time,
                        showItem.schedule.days.joinToString()
                    )

                } else {
                    Toast.makeText(this@ShowTvDetailActivity, "Something broke ${response.message()}", Toast.LENGTH_LONG).show()
                }
            }

            override fun onFailure(call: Call<MySchedule.Show>, t: Throwable) {
                Toast.makeText(this@ShowTvDetailActivity, "Something went wrong $t", Toast.LENGTH_LONG).show()
            }
        ...
        requestCall.enqueue(object : Callback<List<Cast>> {
            override fun onResponse(call: Call<List<Cast>>, response: Response<List<Cast>>) {
                if(response.isSuccessful){
                    ...
                    Toast.makeText(this@ShowTvDetailActivity, "Something went wrong ${response.message()}", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<List<Cast>>, t: Throwable) {
                Toast.makeText(this@ShowTvDetailActivity, "Something went wrong ${t}", Toast.LENGTH_SHORT).show()
            }
